...
function RandomTheme(){
  var GoogleMaterialDesignColorSwatch=
  ...
  ["#f1f8e9","#dcedc8","#c5e1a5","#aed581","#9ccc65","#8bc34a","#7cb342","#689f38","#558b2f","#33691e","#ccff90","#b2ff59","#76ff03","#64dd17"],//Lime Green
...
$("body").css('background-color' , GoogleMaterialDesignColorSwatch[randomColor][5]);
$("button").css('background-color' , GoogleMaterialDesignColorSwatch[Math.floor((Math.random() * 16))][12]);
$(".card").css('background-color' , GoogleMaterialDesignColorSwatch[randomColor][0]);
$(".js-quote").animate({
            backgroundColor: GoogleMaterialDesignColorSwatch[randomColor][7],

        });

$(".js-author").animate({
            color: GoogleMaterialDesignColorSwatch[randomColor][7],
        });
}

function mycallback(data){

  if(data[0].content.length>=240){
    $.ajax({
      type:"GET",
...
myAuthor=data[0].title;
myQuote=data[0].content;
myQuote= myQuote.replace( "</p>","");//temp fix regex wasnt working well
myQuote= myQuote.replace( "<p>","");
myQuote= myQuote.replace( "&#8217;","'");


$("#quote-content").html("<div class=\"js-quote\"> " + myQuote + " </div> <div class=\"js-author\"> -" + myAuthor+ "</div>");
  RandomTheme();
}
$.ajax({
  type:"GET",
...
$('#tweet-quote').attr('href', 'https://twitter.com/intent/tweet?hashtags=quotes&text=' + encodeURIComponent(  myQuote  + myAuthor));
  });
